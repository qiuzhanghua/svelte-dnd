...
const enum DEFAULT_CLASS {
	MAIN = "svelte",
	DRAGGING = "svelte-dragging",
	DRAGGED = "svelte-dragged"
}
...
export const draggable = (node: HTMLElement, options: DragOptions = {}) => {
	...
	const callEvent = (eventName: "svelte:start" | "svelte" | "svelte:end", fn: typeof onDrag) => {
	...
	function fireSvelteDragStartEvent() {
		callEvent("svelte:start", onDragStart);
	}

	function fireSvelteDragEndEvent() {
		callEvent("svelte:end", onDragEnd);
	}

	function fireSvelteDragEvent() {
		callEvent("svelte", onDrag);
	}
